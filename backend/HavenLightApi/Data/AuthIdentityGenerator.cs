...
public static class AuthIdentityGenerator
{
        ...
        IConfiguration configuration)
    {
        ...
        foreach (var roleName in new[] { AuthRoles.User, AuthRoles.Admin, AuthRoles.Donor })
        {
        ...
        var adminEmail = adminSection["Email"] ?? "admin@havenlight.ph";
        var adminPassword = adminSection["Password"] ?? "Admin@Haven2026!";
        ...
        if (adminUser == null)
        {
            ...
            {
                throw new Exception(
                    $"Failed to create admin user {adminEmail}: {string.Join(", ", createUserResult.Errors.Select(e => e.Description))}");
            }
        }
        ...
        if (!await userManager.IsInRoleAsync(adminUser, AuthRoles.Admin))
        {
            ...
            {
                throw new Exception(
                    $"Failed to add admin user {adminEmail} to role {AuthRoles.Admin}: {string.Join(", ", addToRoleResult.Errors.Select(e => e.Description))}");
            }
        }
    }
}